...
Run:  GEMINI_API_KEY=... python app.py   →   http://localhost:5000
Deps: pip install flask requests google-genai
"""
...
def rate_limited(client_ip):
    """Sliding-window limiter for the endpoint that spends API credit."""
    now = time.monotonic()
    with _rate_lock:
...
def gemini_grade(host, rows):
    """Ask Gemini Flash for a grade. Returns a dict or None on any failure —
    ...
    try:
        ...
        if grade not in "ABCDEF" or not grade:
            raise ValueError(f"Gemini returned an invalid grade: {data.get('grade')!r}")
    ...
    except Exception as exc:  # any Gemini failure degrades to headers-only
        app.logger.warning("Gemini grading unavailable: %s", exc)
        return None
...
def scan():
    ...
    if rate_limited(request.remote_addr):
        return render_template_string(
            TEMPLATE, scan=None, target_input=raw,
          ...
          <div class="bg-[#0D1117] border border-[#30363D] rounded-xl p-8 md:col-span-8 flex flex-col justify-between shadow-lg">
            <div class="space-y-4">
              ...
              <p class="text-xs text-slate-400 leading-relaxed">
                Grade and remediation guidance are generated by Gemini Flash from the live response headers of the final (post-redirect) URL. Validate every suggested header in a staging environment before deploying to production.
              </p>
              {% else %}
              ...
              <p class="text-xs text-slate-400 leading-relaxed">
                The Gemini request failed or no GEMINI_API_KEY is configured. Set the environment variable and re-run the scan to receive a letter grade, verdict, and remediation recipes.
              </p>
              {% endif %}
            </div>
